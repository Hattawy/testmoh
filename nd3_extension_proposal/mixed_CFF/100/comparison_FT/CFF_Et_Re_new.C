void CFF_Et_Re_new()
{
  ...
  sprintf(psnamebig1,"CFF_etre.eps");
  sprintf(gifnamebig1,"CFF_etre.pdf");
  ...
  for(Int_t j=NQ2;j>0;j--)
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  for(Int_t k=1;k<NT+1;k++)
		...
		if(Eretm[j-1][i-1][k-1]!=0 && Eretp[j-1][i-1][k-1]!=0 && TMath::IsNaN(Ret[j-1][i-1][k-1])!=1)
		  {
		...
		if(i==4 && k==1)
		  {
		    Iht[j-1][i-1][k-1]=-1000.;
		    Ih[j-1][i-1][k-1]=-1000.;
		    Ie[j-1][i-1][k-1]=-1000.;
		    Rht[j-1][i-1][k-1]=-1000.;
		    Rh[j-1][i-1][k-1]=-1000.;
		    Re[j-1][i-1][k-1]=-1000.;
		    Ret[j-1][i-1][k-1]=-1000.;
		    Iht_good[j-1][i-1][k-1]=-1000.;
		    Ih_good[j-1][i-1][k-1]=-1000.;
		    Ie_good[j-1][i-1][k-1]=-1000.;
		    Rht_good[j-1][i-1][k-1]=-1000.;
		    ...
		    Re_good[j-1][i-1][k-1]=-1000.;
		    Ret_good[j-1][i-1][k-1]=-1000.;
		    Hrevgg[j-1][i-1][k-1]=-1000.;
		    Himvgg[j-1][i-1][k-1]=-1000.;
		    Htrevgg[j-1][i-1][k-1]=-1000.;
		    Htimvgg[j-1][i-1][k-1]=-1000.;
		    Erevgg[j-1][i-1][k-1]=-1000.;
		    Eimvgg[j-1][i-1][k-1]=-1000.;
		    Etrevgg[j-1][i-1][k-1]=-1000.;
		  }
		if (!file.good()) break; 
	      }
	      file.close();
	  ...
	  gr2=new TGraphAsymmErrors(NT,t_vec,Ret[j-1][i-1],err_t,err_t,Eretm[j-1][i-1],Eretp[j-1][i-1]);
	  gr2->SetMarkerColor(1);
	  gr2->SetMarkerStyle(21);
	  gr2->SetMarkerSize(0.5);
	  gr2->SetTitle(titolo);
	  gr2->GetXaxis()->SetTitle("-t (GeV^{2})");
	  gr2->GetYaxis()->SetTitle("Re(#tilde{E})");
	  gr2->SetMaximum(300);
	  gr2->SetMinimum(-500);
	  //gr2->Draw("AP,same");
	  gr2_good=new TGraphAsymmErrors(NT,t_vec,Ret_good[j-1][i-1],err_t,err_t,Eretm_good[j-1][i-1],Eretp_good[j-1][i-1]);
          int n = gr2_good->GetN();
	  ...
	  gr2_good->SetTitle(titolo);
          ...
          gr2_good->GetYaxis()->SetTitle("#tilde{E}_{Re}(n)");
          gr2_good->SetMaximum(300);
          gr2_good->SetMinimum(-600);
	  ...
	  gr2_vgg=new TGraph(NT,t_vec,Etrevgg[j-1][i-1]);
