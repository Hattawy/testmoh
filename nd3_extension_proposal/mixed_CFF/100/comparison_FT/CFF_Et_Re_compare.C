void CFF_Et_Re_compare()
{
  ...
  Double_t t_vec_50[NT]={0.12,0.37,0.67,1.02};
  ...
  sprintf(psnamebig1,"CFF_etre_compare.eps");
  sprintf(gifnamebig1,"CFF_etre_compare_FT.pdf");
  sprintf(gifnamebig2,"CFF_etre_50days.pdf");
    
  TCanvas *g1= new TCanvas(canvas1,canvas1,1000,800);
  TCanvas *g2= new TCanvas(canvas2,canvas2,1000,800);
  ...
  while (1) {
    ...
    if(error[a-1][b-1][c-1]>1.5*TMath::Abs(value[a-1][b-1][c-1]))
      {
	value[a-1][b-1][c-1]=-1000.;
	error[a-1][b-1][c-1]=0.;
      }
    if (!file.good()) break; 
  }
  ...
  while (1) {
    ...
    if(TMath::Abs(error_50[a-1][b-1][c-1])>1.5*TMath::Abs(value_50[a-1][b-1][c-1]))
      {      
	value_50[a-1][b-1][c-1]=-1000.;
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      int n = gr1_good->GetN();
	      double* y1 = gr1_good->GetY();
	      int locmax = TMath::LocMax(n,y1);
	      double tmax = y1[locmax];
	      int locmin = TMath::LocMin(n,y1);
	      double tmin = y1[locmin];
	      double ymax = y1[locmax]+Eihp_good[j-1][i-1][locmax]+50;
	      double ymin = y1[locmin]-Eihm_good[j-1][i-1][locmin]-50;
	      if(tmin==-1000 && j==3) ymin=-200;
	      if(tmin==-1000 && j==2 && i==1) ymin=-100;
	      if(tmin==-1000 && j==2 && i==2) ymin=-150;
	      if(tmin==-1000 && j==2 && i==3) {ymin=-350;ymax=100;}
	      if(tmin==-1000 && j==1 && i==2) ymin=-500;
	      if(tmin==-1000 && j==1 && i==3) {ymin=-60;ymax=20;}

	      //if(tmin==-1000) ymin=-200;
	      if(tmax>-1000 && !(j==3 && i==4) && !(j==2 && i==4)))
		{
		  gr1_good_50->SetTitle(titolo);
		  gr1_good_50->GetXaxis()->SetTitle("-t (GeV^{2})");
		  gr1_good_50->GetXaxis()->SetLabelOffset(0.001);
		  gr1_good_50->GetYaxis()->SetTitle("#tilde{E}_{Re}(n)");
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      int locmin = TMath::LocMin(n,y1);
	      double tmin = y1[locmin];
	      double ymax = y1[locmax]+Eihp_good_50[j-1][i-1][locmax]+50;
	      double ymin = y1[locmin]-Eihm_good_50[j-1][i-1][locmin]-50;
	      if(tmin==-1000 && j==1 && i==1) ymin=-500;
	      if(tmin==-1000 && j==4) ymin=-100;
	      if(tmin==-1000 && j==3) ymin=-200;
	      if(tmin==-1000 && j==2 && i==1) ymin=-60;
	      if(tmin==-1000 && j==2 && i==2) ymin=-150;
	      if(tmin==-1000 && j==2 && i==3) {ymin=-50;ymax=20;}
	      if(tmin==-1000 && j==1 && i==2) ymin=-500;
	      if(tmin==-1000 && j==1 && i==3) {ymin=-60;ymax=20;}
	      //if(tmin==-1000) ymin=-200;
	      if(tmax>-1000)
		{
		  ...
		  l1->SetLineStyle(2); 
		  l1->Draw("same");
		  gr1_good_50->SetMarkerColor(4);
		  gr1_good_50->SetLineColor(4);
		  gr1_good_50->SetMarkerSize(0.5);
		  gr1_good_50->SetMarkerStyle(21);
		  gr1_good_50->Draw("AP,same");
		}
