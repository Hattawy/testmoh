void CFF_Ht_Re_new()
{
  ...
  sprintf(psnamebig1,"CFF_htre.pdf");
  ...
  for(Int_t j=NQ2;j>0;j--)
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  int n = gr4_good->GetN();
          double* y = gr4_good->GetY();
          int locmax = TMath::LocMax(n,y);
          double tmax = y[locmax];
          //      cout<<tmax<<endl;                                                                                    
          if(tmax>-100)
            {
          gr4_good->SetTitle(titolo);
          gr4_good->GetXaxis()->SetTitle("-t (GeV^{2})");
          gr4_good->GetYaxis()->SetTitle("#tilde{H}_{Re}(n)");
          gr4_good->SetMaximum(4.);
          gr4_good->SetMinimum(-3.);

	  gr4_good->SetLineColor(4);
	  gr4_good->SetMarkerColor(4);	  
	  gr4_good->SetMarkerStyle(21);
	  gr4_good->SetMarkerSize(0.5);
	  gr4_good->Draw("AP,same");
