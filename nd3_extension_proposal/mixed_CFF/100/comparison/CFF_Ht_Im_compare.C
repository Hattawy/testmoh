void CFF_Ht_Im_compare()
{
  ...
  sprintf(psnamebig1,"CFF_htim_compare.eps");
  sprintf(gifnamebig1,"CFF_htim_compare.pdf");
  sprintf(gifnamebig2,"CFF_htim_50days.pdf");
  sprintf(psnamebig2,"CFF_htim_50days.eps");
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      double ymax = y1[locmax]+error[j-1][i-1][locmax]+0.5;
	      double ymin = y1[locmin]-error[j-1][i-1][locmin]-1;
	      if(tmin==-100) ymin=-1.5;
	      if(i==3 && j==1) {ymin = -0.3;ymax= 0.3;}
	      if(i==2 && j==1) {ymin = -0.3;ymax= 0.5;}
	      if(tmax>-100)
		{
		  gr1_good_50->SetTitle(titolo);
		  gr1_good_50->GetXaxis()->SetTitle("-t (GeV^{2})");
		  gr1_good_50->GetXaxis()->SetLabelOffset(0.001);
		  gr1_good_50->GetYaxis()->SetTitle("#tilde{H}_{Im}(n)");
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      int locmin = TMath::LocMin(n,y1);
	      double tmin = y1[locmin];
	      double ymax = y1[locmax]+error_50[j-1][i-1][locmax]+0.5;
