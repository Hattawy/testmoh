void CFF_H_Im_compare()
{
  ...
  sprintf(psnamebig1,"CFF_him_compare.eps");
  sprintf(gifnamebig1,"CFF_him_compare.pdf");
  sprintf(gifnamebig2,"CFF_him_50days.pdf");
  sprintf(psnamebig2,"CFF_him_50days.eps");
  ...
  while (1) {
    ...
    if(error[a-1][b-1][c-1]>5.*TMath::Abs(value[a-1][b-1][c-1]))
      {
  ...
  while (1) {
    ...
    if(error_50[a-1][b-1][c-1]>5.*TMath::Abs(value_50[a-1][b-1][c-1]))
      {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(tmin==-100 && i!=4) ymin=-0.5;
	      if(tmin==-100 && i==4 && j==2) {ymin=-1; ymax=1;}
	      if(tmin==-100 && i==4 && j==4) {ymin=-2; ymax=1;}	
