void CFF_Ht_Re_compare()
{
  ...
  sprintf(psnamebig1,"CFF_htre_compare.eps");
  sprintf(gifnamebig1,"CFF_htre_compare.pdf");
  sprintf(gifnamebig2,"CFF_htre_50days.pdf");
  sprintf(psnamebig2,"CFF_htre_50days.eps");
  ...
  while (1) {
    ...
    if(error[a-1][b-1][c-1]>100.*TMath::Abs(value[a-1][b-1][c-1]))
      {
  ...
  while (1) {
    ...
    if(error_50[a-1][b-1][c-1]>100.*TMath::Abs(value_50[a-1][b-1][c-1]))
      {
