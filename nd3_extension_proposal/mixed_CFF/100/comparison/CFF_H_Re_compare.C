void CFF_H_Re_compare()
{
  ...
  sprintf(psnamebig1,"CFF_hre_compare.eps");
  sprintf(gifnamebig1,"CFF_hre_compare.pdf");
  sprintf(gifnamebig2,"CFF_hre_50days.pdf");
  sprintf(psnamebig2,"CFF_hre_50days.eps");
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      double ymax = y1[locmax]+Eihp_good[j-1][i-1][locmax]+1;
	      double ymin = y1[locmin]-Eihm_good[j-1][i-1][locmin]-1;
