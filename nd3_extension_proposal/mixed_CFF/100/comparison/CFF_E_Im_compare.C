void CFF_E_Im_compare()
{
  ...
  sprintf(psnamebig1,"CFF_eim_compare.eps");
  sprintf(gifnamebig1,"CFF_eim_compare.pdf");
  sprintf(gifnamebig2,"CFF_eim_50days.pdf");
  sprintf(psnamebig2,"CFF_eim_50days.eps");
  ...
  while (1) {
    ...
    if(error[a-1][b-1][c-1]>1000.*TMath::Abs(value[a-1][b-1][c-1]))
      {
  ...
  while (1) {
    ...
    if(error_50[a-1][b-1][c-1]>1000.*TMath::Abs(value_50[a-1][b-1][c-1]))
      {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(tmin==-100) ymin=-1.5;
	      if(tmin==-100 && i==3 && j==3) {ymax = 2;ymin=-2;}
	      if(i==3 && j==2) {ymax = 5;ymin=-5;}
	      if(j==4 && i==3) {ymax = 4;}
	      if(i==4 && j==2) {ymax = 0.5;ymin=-0.8;}
	      if(i==2 && j==3) {ymax = 3;ymin=-3;}
	      if(i==1 && j==1) {ymax = 2;ymin=-0.5;}
	      if(i==1 && j==2) {ymax = 4;ymin=-8;}
	      if(i==2 && j==2) {ymax = 5;ymin=-2;}
	      if(tmax>-100)
		{
		  gr1_good_50->SetTitle(titolo);
		  gr1_good_50->GetXaxis()->SetTitle("-t (GeV^{2})");
		  gr1_good_50->GetXaxis()->SetLabelOffset(0.001);
		  gr1_good_50->GetYaxis()->SetTitle("E_{Im}(n)");
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      int locmin = TMath::LocMin(n,y1);
	      double tmin = y1[locmin];
	      double ymax = y1[locmax]+Eihp_good_50[j-1][i-1][locmax]+1;
	      double ymin = y1[locmin]-Eihm_good_50[j-1][i-1][locmin]-1;
	      if(tmin==-100) ymin=-1.5;
	      if(tmin==-100 && i==3 && j==3) {ymax = 2;ymin=-2;}
	      if(i==3 && j==2) {ymax = 5;ymin=-5;}
	      if(j==4 && i==3) {ymax = 4;}
