void CFF_E_Im_compare3()
{
  ...
  sprintf(gifnamebig1,"CFF_eim_compare3.pdf");
  sprintf(psnamebig1,"CFF_eim_compare3.eps");
  ...
  ifstream file_noft = (out_e_im_noft);
  while (1) {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(i==1 && j==1) {ymax = 2;ymin=-0.8;}
	      if(i==1 && j==2) {ymax = 4;ymin=-8;}
	      if(i==2 && j==2) {ymax = 5;ymin=-2;}
	      if(i==2 && j==1) {ymax = 1.5;ymin=-0.5;}
	      if(tmax>-100)
		{
