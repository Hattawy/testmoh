void CFF_E_Re_compare3()
{
  ...
  sprintf(gifnamebig1,"CFF_ere_compare3.pdf");
  sprintf(psnamebig1,"CFF_ere_compare3.eps");
  ...
  ifstream file_noft = (out_e_re_noft);
  while (1) {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(tmin==-100 && i==2) {ymin=-1;ymax=1.5;}
	      if(tmax>-100)
		{
