void CFF_Et_Re_compare3()
{
  ...
  sprintf(gifnamebig1,"CFF_etre_compare3.pdf");
  sprintf(psnamebig1,"CFF_etre_compare3.eps");
  ...
  while (1) {
    ...
    if(error[a-1][b-1][c-1]>10.*TMath::Abs(value[a-1][b-1][c-1]))
      {
	value[a-1][b-1][c-1]=-1000.;
	error[a-1][b-1][c-1]=0.;
      }
    if(value[a-1][b-1][c-1]==-100.)
      {
	value[a-1][b-1][c-1]=-1000.;
	error[a-1][b-1][c-1]=0.;
      }
  ...
  ifstream file_noft = (out_et_re_noft);
  while (1) {
    ...
    if(error_noft[a-1][b-1][c-1]>10.*TMath::Abs(value_noft[a-1][b-1][c-1]))
      {
	value_noft[a-1][b-1][c-1]=-1000.;
	error_noft[a-1][b-1][c-1]=0.;
      }
    if(value_noft[a-1][b-1][c-1]==-100.)
      {
	value_noft[a-1][b-1][c-1]=-1000.;
	error_noft[a-1][b-1][c-1]=0.;
      }
  ...
  while (1) {
    ...
    if(error_50[a-1][b-1][c-1]>10.*TMath::Abs(value_50[a-1][b-1][c-1]))
      {
	value_50[a-1][b-1][c-1]=-1000.;
	error_50[a-1][b-1][c-1]=0.;
      }
    if(value_50[a-1][b-1][c-1]==-100.)
      {
	value_50[a-1][b-1][c-1]=-1000.;
	error_50[a-1][b-1][c-1]=0.;
      }
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(j==4 && i==4) {ymin=-80;ymax=20;}
	      if(j==4 && i==3) {ymin=-240;ymax=70;}
	      if(j==4 && i==2) {ymin=-150;ymax=50;}
	      ...
	      if(j==3 && i==2) {ymin=-200;ymax=20;}
	      if(j==3 && i==3) {ymin=-300;ymax=100;}
	      if(j==2 && i==1) {ymin=-450;ymax=150;}
	      if(j==2 && i==2) {ymin=-140;ymax=20;}
	      if(j==2 && i==3) {ymin=-320;ymax=80;}
	      if(j==2 && i==4) {ymin=-10;ymax=10;}
	      if(j==1 && i==1) {ymin=-450;ymax=50;}
	      if(j==1 && i==2) {ymin=-450;ymax=50;}
	      if(j==1 && i==3) {ymin=-50;ymax=10;}
	      if(tmax>-100)
		{
