void CFF_H_Im_compare3()
{
  ...
  sprintf(gifnamebig1,"CFF_him_compare3.pdf");
  sprintf(psnamebig1,"CFF_him_compare3.eps");
  ...
  ifstream file_noft = (out_h_im_noft);
  while (1) {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  for(Int_t k=1;k<NT+1;k++)
	    {	         
	      char titolo[30];
	      sprintf(titolo,"");
	      if((j==2 && i==1)||(j==3 && i==2) || (j==2 && i==2) || (j==4 && i==3)){
		cout<<"100 FT "<<"j "<<j<<" i "<<i<<" k "<<k<<" value "<<value[j-1][i-1][k-1]<<" error "<<error[j-1][i-1][k-1]<<endl;
		cout<<"100 noFT "<<"j "<<j<<" i "<<i<<" k "<<k<<" value "<<value_noft[j-1][i-1][k-1]<<" error "<<error_noft[j-1][i-1][k-1]<<endl;
		cout<<"50 "<<"j "<<j<<" i "<<i<<" k "<<k<<" value "<<value_50[j-1][i-1][k-1]<<" error "<<error_50[j-1][i-1][k-1]<<endl;
		cout<<"                   "<<endl;
	      }
	    }
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(tmin==-100 && i!=4) ymin=-0.5;
	      if(tmin==-100 && i==4 && j==2) {ymin=-3; ymax=2;}
	      if(tmin==-100 && i==4 && j==4) {ymin=-2; ymax=1;}	      
	      if(i==1 && j==2) {ymax=8;ymin=-2;}
	      if(j==4 && i==2){ymax=6;ymin=-1.5;}
	      if(j==4 && i==3){ymax=3.5;ymin=-0.5;}
	      if(j==3 && i==2){ymax=4;ymin=-1.5;}
	      if(j==3 && i==3){ymax=1;ymin=-0.5;}
	      if(j==2 && i==2){ymax=2;ymin=-1.;}
	      if(j==2 && i==3){ymax=2.5;ymin=-3.;}
	      if(j==1 && i==2){ymax=0.3;ymin=-0.3.;}
	      if(j==1 && i==3){ymax=0.3;ymin=-0.5.;}
	      if(tmax>-100)
		{
		  gr1_good_50->SetTitle(titolo);
		  gr1_good_50->GetXaxis()->SetTitle("-t (GeV^{2})");
		  gr1_good_50->GetXaxis()->SetLabelOffset(0.001);
		  gr1_good_50->GetYaxis()->SetTitle("H_{Im}(n)");
