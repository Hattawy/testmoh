void CFF_Ht_Re_compare3()
{
  ...
  sprintf(gifnamebig1,"CFF_htre_compare3.pdf");
  sprintf(psnamebig1,"CFF_htre_compare3.eps");
  ...
  ifstream file_noft = (out_ht_re_noft);
  while (1) {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(i==1 && j==1) {ymin = -3; ymax = 4;}
	      if(i==2 && j==1) {ymin = -5; ymax = 2;}
	      if(i==2 && j==2) {ymin = -1; ymax = 3;}
	      if(tmax>-100)
		{
