void CFF_Ht_Im_compare3()
{
  ...
  sprintf(gifnamebig1,"CFF_htim_compare3.pdf");
  sprintf(psnamebig1,"CFF_htim_compare3.eps");
  ...
  ifstream file_noft = (out_ht_im_noft);
  while (1) {
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  if(!(j==4 && i==1) && !(j==3 && i==1) && !(j==1 && i==4))
	    {	      
	      ...
	      if(i==1 && j==1) {ymin = -1.5;ymax= 0.5;}
	      if(i==3 && j==1) {ymin = -0.3;ymax= 0.2;}
	      if(i==2 && j==1) {ymin = -1;ymax= 0.5;}
	      if(i==1 && j==2) {ymin = -1.5.;ymax= 1.;}
	      if(i==2 && j==2) {ymin = -1.5;ymax= 0.5;}
	      if(i==2 && j==3) {ymin = -1.5;ymax= 1.;}
	      if(tmax>-100)
		{
