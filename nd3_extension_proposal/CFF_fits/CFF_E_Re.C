void CFF_E_Re()
{
  ...
  sprintf(psnamebig1,"CFF_ere.eps");
  sprintf(gifnamebig1,"CFF_ere.png");
  ...
  for(Int_t j=NQ2;j>0;j--)
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  for(Int_t k=1;k<NT+1;k++)
		...
		if(Erem[j-1][i-1][k-1]!=0 && Erep[j-1][i-1][k-1]!=0 && TMath::IsNaN(Re[j-1][i-1][k-1])!=1))
		  {
	  ...
	  gr2=new TGraphAsymmErrors(NT,t_vec,Re[j-1][i-1],err_t,err_t,Erem[j-1][i-1],Erep[j-1][i-1]);
	  gr2->SetMarkerColor(1);
	  gr2->SetMarkerStyle(21);
	  gr2->SetMarkerSize(0.5);
	  gr2->SetTitle(titolo);
	  gr2->GetXaxis()->SetTitle("-t (GeV^{2})");
	  gr2->GetYaxis()->SetTitle("Re(E)");
	  gr2->SetMaximum(30);
	  gr2->SetMinimum(-20);
	  gr2->Draw("AP,same");
	  gr2_good=new TGraphAsymmErrors(NT,t_vec,Re_good[j-1][i-1],err_t,err_t,Erem_good[j-1][i-1],Erep_good[j-1][i-1]);
	  gr2_good->SetMarkerColor(4);
	  gr2_good->SetMarkerStyle(24);
	  gr2_good->SetMarkerSize(1.2);
	  gr2_good->Draw("P,same");
	  gr2_vgg=new TGraph(NT,t_vec,Erevgg[j-1][i-1]);
