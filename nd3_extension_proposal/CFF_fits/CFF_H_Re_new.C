void CFF_H_Re_new()
{
  ...
  sprintf(psnamebig1,"CFF_hre.eps");
  sprintf(gifnamebig1,"CFF_hre.pdf");
  ...
  for(Int_t j=NQ2;j>0;j--)
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  for(Int_t k=1;k<NT+1;k++)
		...
		if(Erhm[j-1][i-1][k-1]!=0 && Erhp[j-1][i-1][k-1]!=0 && TMath::IsNaN(Rh[j-1][i-1][k-1])!=1)
		  {
	  ...
	  gr3=new TGraphAsymmErrors(NT,t_vec,Rh[j-1][i-1],err_t,err_t,Erhm[j-1][i-1],Erhp[j-1][i-1]);
	  gr3->SetMarkerColor(1);
	  gr3->SetMarkerStyle(21);
	  gr3->SetMarkerSize(0.5);
	  gr3->SetTitle(titolo);
	  gr3->GetXaxis()->SetTitle("-t (GeV^{2})");
	  gr3->GetYaxis()->SetTitle("Re(H)");
	  gr3->SetMaximum(5);
	  gr3->SetMinimum(-8);
	  //	  gr3->Draw("AP,same");
	  gr3_good=new TGraphAsymmErrors(NT,t_vec,Rh_good[j-1][i-1],err_t,err_t,Erhm_good[j-1][i-1],Erhp_good[j-1][i-1]);
          int n = gr3_good->GetN();
          double* y = gr3_good->GetY();
          ...
          gr3_good->SetMarkerStyle(21);
          gr3_good->SetLineColor(4);
          gr3_good->SetTitle(titolo);
          gr3_good->GetXaxis()->SetTitle("-t (GeV^{2})");
          gr3_good->GetYaxis()->SetTitle("H_{Re}(n)");
	  gr3_good->SetMarkerColor(4);
	  gr3_good->SetMarkerStyle(21);
	  gr3_good->SetMarkerSize(0.5);
          gr3_good->SetMaximum(8);
          gr3_good->SetMinimum(-8);
	  gr3_good->Draw("AP,same");
	  gr3_vgg=new TGraph(NT,t_vec,Hrevgg[j-1][i-1]);
	  gr3_vgg->SetMarkerColor(2);
	  gr3_vgg->SetMarkerStyle(25);
	  gr3_vgg->SetMarkerSize(1);
	  //	  gr3_vgg->Draw("P,same");
	  TLine *l1;
