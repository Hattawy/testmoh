void CFF_Ht_Im_new()
{
  ...
  sprintf(psnamebig1,"CFF_htim.pdf");
  sprintf(gifnamebig1,"CFF_htim.pdf");
  ...
  for(Int_t j=NQ2;j>0;j--)
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  for(Int_t k=1;k<NT+1;k++)
		...
		if(Eihtm[j-1][i-1][k-1]!=0 && Eihtp[j-1][i-1][k-1]!=0 && TMath::IsNaN(Iht[j-1][i-1][k-1])!=1))
		  {
      ...
      if(!(j==4 && i==1) && !(j==3 && i==1)){
	  // gr1=new TGraphAsymmErrors(NT,t_vec,Iht[j-1][i-1],err_t,err_t,Eihtm[j-1][i-1],Eihtp[j-1][i-1]);
	  // gr1->SetMarkerColor(1);
	  // gr1->SetMarkerStyle(21);
	  // gr1->SetMarkerSize(0.5);
	  // gr1->SetTitle(titolo);
	  // gr1->GetXaxis()->SetTitle("-t (GeV^{2})");
	  // gr1->GetYaxis()->SetTitle("Im(#tilde{H})");
	  // gr1->SetMaximum(1);
	  // gr1->SetMinimum(-2);
	  // gr1->Draw("AP,SAME");
	  gr1_good=new TGraphAsymmErrors(NT,t_vec,Iht_good[j-1][i-1],err_t,err_t,Eihtm_good[j-1][i-1],Eihtp_good[j-1][i-1]);
	  int n = gr1_good->GetN();
	  double* y = gr1_good->GetY();
	  int locmax = TMath::LocMax(n,y);
	  double tmax = y[locmax];
	  //      cout<<tmax<<endl;                                                                                       
	  if(tmax>-100)
	    {
	  gr1_good->GetXaxis()->SetTitle("-t (GeV^{2})");
	  gr1_good->GetYaxis()->SetTitle("#tilde{H}_{Im}(n)");
	  gr1_good->SetTitle(titolo);
	  gr1_good->SetMaximum(1);
	  gr1_good->SetMinimum(-2);
	  gr1_good->SetLineColor(4);
	  gr1_good->SetMarkerColor(4);
	  gr1_good->SetMarkerStyle(21);
	  gr1_good->SetMarkerSize(0.5);
	  gr1_good->Draw("AP,same");
	  gr1_vgg=new TGraph(NT,t_vec,Htimvgg[j-1][i-1]);
	  gr1_vgg->SetMarkerColor(2);
	  gr1_vgg->SetMarkerStyle(25);
	  gr1_vgg->SetMarkerSize(1);
	  //	  gr1_vgg->Draw("P,same");
	  TLine *l1;
