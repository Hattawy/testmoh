void CFF_H_Re()
{
  ...
  sprintf(gifnamebig1,"CFF_hre.png");
      ...
      for(Int_t i=1;i<NXB+1;i++)
	{
	  ...
	  for(Int_t k=1;k<NT+1;k++)
	    {	         
	      ...
	      char titolo[30];
	      sprintf(filename,"/projet/nucleon/silvia/nd3_extension_proposal/CFF_fits/fiteg1_%d_%d_%d.dat",j,i,k);
	      sprintf(titolo,"");
	      s=0;
	  ...
	  gr3_good->SetMarkerStyle(24);
	  gr3_good->SetMarkerSize(1.2);
	  gr3_good->Draw("P,same");
