...
gStyle->SetErrorX(0);
 ...
 sprintf(canvas,"g1");
 sprintf(psnamebig,"asym_polndvcs_dsa.eps");
 sprintf(gifnamebig,"asym_polndvcs_dsa.jpg");
	 ...
	 for(Int_t k=1;k<NT+1;k++)
	   {
		 ...
		 a1 = (upup1+dndn1-dnup1-updn1)/(upup1+updn1+dnup1+dndn1);
		 cout<<"a1 "<<a1<<endl;
	       }
	       else a1 = -1000;
	       //		X1[j-1][i-1][k-1][s]=x1;
	     ...
	     if(TMath::Abs(histo->GetBinContent(6))>0.00001)){
