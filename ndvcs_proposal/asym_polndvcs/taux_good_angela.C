...
gStyle->SetErrorX(0);
 ...
 sprintf(canvas,"g1");
 sprintf(psnamebig,"asym_newcuts_new_angela.eps");
 sprintf(gifnamebig,"asym_newcuts_new_angela.jpg");
	 ...
	 for(Int_t k=1;k<NT+1;k++)
	   {
	     ...
	     if(histo->GetMaximum()>0.00001)){
	       //	       if(k!=2)
	       //{
	        histo->SetMaximum(0.2);
