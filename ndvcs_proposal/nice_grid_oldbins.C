...
TFile *data = new TFile("/projet/nucleon/Baptiste/genepi/baptiste_code_AVIRER_stat_4/ana.root");
 ...
 for(Int_t r=1;r<4;r++)
   {
	 ...
	 sprintf(psnamebig,"plot_%s_%d.eps",type,k);
	 ...
	 for(Int_t j=NQ2;j>0;j--)
	   {
		 ...
		 sprintf(new_name,"p%d",j*100+i*10+k);
		 ...
		 if(data_hist->Integral()>0){
		   ...
		   if(r==2)
		       ...
		       for(Int_t m=1;m<NPHI+1;m++)
			 {
			   ...
			   //			   new_hist->SetTitle("");
			   //here put counts in output txt file
			   cout<<counts<<endl;
			   cr_file << counts << endl;
			 }
		       //		       new_hist->Draw("e");
